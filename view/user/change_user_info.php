<?php
    ...
    if($firstName == "NULL" || $lastName == "NULL" || $phone_number == "NULL"){
        redirect_to("userProfile.php?fill=false");
    }

    if(!checkName($_POST['firstName']) || !checkName($_POST['lastName'])){
        redirect_to("userProfile.php?valid=false");
    }
    ...
    if($email != "NULL"){
        ...
        if( intval($row['c']) > 0){
            redirect_to("userProfile.php?emailE=false");
            exit;
        }
    }   
    ...
    $row = mysqli_fetch_assoc($result);
    if( intval($row['c']) > 0){
        redirect_to("userProfile.php?phoneE=false");
        exit;
    }
    ...
    if(mysqli_query($db, $sql)){
        redirect_to("userProfile.php?updated=true");
        exit;
    }else{
        redirect_to("userProfile.php?notUp=true");
        exit;
    }
